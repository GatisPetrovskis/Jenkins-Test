...
def deployService(envName, port) {
    ...
    bat 'cd python-greetings && python -m venv venv'
    bat 'cd python-greetings && venv\\Scripts\\python -m pip install -r requirements.txt'
    bat "cd python-greetings && pm2 delete greetings-app-${envName} & set \"errorlevel=0\""
    bat "cd python-greetings && set PORT=${port} && pm2 start app.py --name greetings-app-${envName} --interpreter \"%CD%\\venv\\Scripts\\python.exe\""
}
